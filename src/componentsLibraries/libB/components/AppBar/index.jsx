...
import { LibrariesContext } from "@/contexts/LibrariesProvider/index.jsx";
...
const menuItems = [[
    ...
    { text: "Settings", icon: SettingsIcon, action: () => alert("Settings") }
], [
        ...
        <nav className={styles.appBar.appBar} >
            <ul key={0} className={styles.appBar.ul} >
                <li onClick={() => setOpenNavBar(!openNavBar)} className={styles.appBar.navigationButton}>
                    <svg version="1.1" id="_x32_" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512" xmlSpace="preserve" style={{ width: '50%', height: '50%' }}> <g> <circle className={styles.appBar.fillSvg} cx={48} cy={64} r={48} /> <rect x={160} y={16} className={styles.appBar.fillSvg} width={352} height={96} /> <circle className={styles.appBar.fillSvg} cx={48} cy={256} r={48} /> <rect x={160} y={208} className={styles.appBar.fillSvg} width={352} height={96} /> <circle className={styles.appBar.fillSvg} cx={48} cy={448} r={48} /> <rect x={160} y={400} className={styles.appBar.fillSvg} width={352} height={96} /> </g></svg>
                </li>
            ...
            {menuItems.map((menuItem, i) =>
                ...
                <ul key={i} className={styles.appBar.ul}>
                    {menuItem.map((item) => <li key={item.text}>
                        <Button
                            key={item.text}
                            className={styles.appBar.buttonCustomStyle}
                            onClick={item?.action}
